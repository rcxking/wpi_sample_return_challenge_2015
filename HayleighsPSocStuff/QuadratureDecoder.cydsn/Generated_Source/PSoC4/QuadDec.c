...
void QuadDec_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        QuadDec_CONTROL_REG = QuadDec_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        QuadDec_TRIG_CONTROL1_REG  = QuadDec_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        QuadDec_SetInterruptMode(QuadDec_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        QuadDec_SetCounterMode(QuadDec_COUNT_DOWN);
        QuadDec_WritePeriod(QuadDec_QUAD_PERIOD_INIT_VALUE);
        QuadDec_WriteCounter(QuadDec_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (QuadDec__QUAD == QuadDec_CONFIG) */

    #if (QuadDec__TIMER == QuadDec_CONFIG)
        QuadDec_CONTROL_REG = QuadDec_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        QuadDec_TRIG_CONTROL1_REG  = QuadDec_TIMER_SIGNALS_MODES;
        ...
        #if (QuadDec_CY_TCPWM_V2 && QuadDec_TIMER_UPDOWN_CNT_USED && !QuadDec_CY_TCPWM_4000)
            QuadDec_WriteCounter(1u);
        #elif(QuadDec__COUNT_DOWN == QuadDec_TC_COUNTER_MODE)
            QuadDec_WriteCounter(QuadDec_TC_PERIOD_VALUE);
        #else
            QuadDec_WriteCounter(0u);
        #endif /* (QuadDec_CY_TCPWM_V2 && QuadDec_TIMER_UPDOWN_CNT_USED && !QuadDec_CY_TCPWM_4000) */
    #endif  /* (QuadDec__TIMER == QuadDec_CONFIG) */

    #if (QuadDec__PWM_SEL == QuadDec_CONFIG)
        QuadDec_CONTROL_REG = QuadDec_CTRL_PWM_BASE_CONFIG;

        #if (QuadDec__PWM_PR == QuadDec_PWM_MODE)
            QuadDec_CONTROL_REG |= QuadDec_CTRL_PWM_RUN_MODE;
            QuadDec_WriteCounter(QuadDec_PWM_PR_INIT_VALUE);
        #else
            QuadDec_CONTROL_REG |= QuadDec_CTRL_PWM_ALIGN | QuadDec_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (QuadDec_CY_TCPWM_V2 && QuadDec_PWM_UPDOWN_CNT_USED && !QuadDec_CY_TCPWM_4000)
                QuadDec_WriteCounter(1u);
            #elif (QuadDec__RIGHT == QuadDec_PWM_ALIGN)
                QuadDec_WriteCounter(QuadDec_PWM_PERIOD_VALUE);
            #else 
                QuadDec_WriteCounter(0u);
            #endif  /* (QuadDec_CY_TCPWM_V2 && QuadDec_PWM_UPDOWN_CNT_USED && !QuadDec_CY_TCPWM_4000) */
        #endif  /* (QuadDec__PWM_PR == QuadDec_PWM_MODE) */

        #if (QuadDec__PWM_DT == QuadDec_PWM_MODE)
            QuadDec_CONTROL_REG |= QuadDec_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (QuadDec__PWM_DT == QuadDec_PWM_MODE) */

        #if (QuadDec__PWM == QuadDec_PWM_MODE)
            QuadDec_CONTROL_REG |= QuadDec_CTRL_PWM_PRESCALER;
        #endif  /* (QuadDec__PWM == QuadDec_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        QuadDec_TRIG_CONTROL1_REG  = QuadDec_PWM_SIGNALS_MODES;
...
*
* Summary:
*  Initializes the QuadDec with default customizer
...
*  calls to the QuadDec_Start() routine.
*
*******************************************************************************/
...
*  QuadDec operates in the continuous mode.
*
* Parameters:
*  oneShotEnable
*   Values:
...
void QuadDec_WriteCompare(uint32 compare)
{
    ...
    #if (QuadDec_CY_TCPWM_4000)
        currentMode = ((QuadDec_CONTROL_REG & QuadDec_UPDOWN_MASK) >> QuadDec_UPDOWN_SHIFT);

        if (((uint32)QuadDec__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)QuadDec__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 QuadDec_ReadCompare(void)
{
        ...
        regVal = QuadDec_COMP_CAP_REG;
        
        if (((uint32)QuadDec__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)QuadDec__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & QuadDec_16BIT_MASK);
    #else
...
void QuadDec_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)QuadDec__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)QuadDec__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
uint32 QuadDec_ReadCompareBuf(void)
{
        ...
        regVal = QuadDec_COMP_CAP_BUF_REG;
